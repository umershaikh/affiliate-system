...
const AdminTotalDeposits = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [acting, setActing] = useState(null);

  const fetchData = () => {
    apiFetch('/api/admin/deposits/all')
      .then(res => res.json())
  ...
  return (
    <div className="admin-page">
      <div className="admin-breadcrumb">🏠 / Admin / All Deposits</div>
      <h2 className="admin-title">All Deposits</h2>
      ...
      <div className="admin-card">
        <div className="admin-card-header">All Deposit Records</div>
        <div className="admin-card-body">
          {loading ? <div className="admin-loading">Loading...</div> : data.length === 0 ? <div className="admin-empty">No deposits</div> : (
            <div className="admin-table-wrap">
              <table className="admin-table">
                <thead><tr><th>ID</th><th>Username</th><th>Email</th><th>Amount</th><th>Method</th><th>TRX ID</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
                <tbody>
                  {data.map(row => (
                    <tr key={row.id}>
                      <td>{row.id}</td><td>{row.username}</td><td>{row.email}</td>
                      <td>Rs. {row.amount}</td><td>{row.paymentMethod}</td><td>{row.trxId || '—'}</td>
...
export default AdminTotalDeposits;
